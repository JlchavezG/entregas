...
$stmt = $pdo->prepare("SELECT * FROM repartidores WHERE id = ?");
$stmt->execute([$id]);
$repartidor = $stmt->fetch();
if (!$repartidor) die('Repartidor no encontrado');
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
    ...
    <main class="main-content">
        <h1 class="page-title">✏️ Editar Repartidor</h1>
        <div class="card">
            <form method="POST" action="guardar_edicion_repartidor.php">
                <input type="hidden" name="id" value="<?= $repartidor['id'] ?>">
                <div class="form-group">
                    <label>Nombre completo</label>
                    <input type="text" name="nombre" value="<?= htmlspecialchars($repartidor['nombre']) ?>" placeholder="Ej. Carlos Méndez" required style="width: 100%; padding: 0.625rem 0.875rem; border: 1px solid var(--border); border-radius: 8px; font-size: 0.95rem; background: var(--bg-card); color: var(--text);">
                </div>
